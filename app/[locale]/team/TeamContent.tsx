...
const memberKeys = ["jurgen", "moritz", "felix"] as const;
      ...
      <div className="grid grid-2 gap-8" style={{ maxWidth: "1200px", margin: "0 auto" }}>
        {memberKeys.map((key) => {
          const member = {
            ...
            avatar: getAvatar(key),
            github: getGithub(key),
            github2: key === "jurgen" ? "https://github.com/jmueller-bit" : undefined,
            linkedin: getLinkedin(key),
          };
...
function getAvatar(key: string): string {
  if (key === "jurgen") return "https://ui-avatars.com/api/?name=J%C3%BCrgen+M%C3%BCller&background=5b9bd5&color=fff&size=256";
  if (key === "moritz") return "https://ui-avatars.com/api/?name=Moritz+M%C3%BCller&background=7dd3c0&color=fff&size=256";
  if (key === "felix") return "https://ui-avatars.com/api/?name=Felix+M%C3%BCller&background=9b8fb8&color=fff&size=256";
  return "";
}

function getGithub(key: string): string {
  if (key === "jurgen") return "https://github.com/foozy74";
  return "#";
}

function getLinkedin(key: string): string {
  if (key === "jurgen") return "https://www.linkedin.com/in/j%C3%BCrgen-m%C3%BCller-b4792a57";
  return "#";
}
      ...
      >
        ...
        {member.github2 && (
          <a
            ...
            onMouseEnter={(e) => {
              e.currentTarget.style.color = "var(--accent-purple, #a855f7)";
              e.currentTarget.style.background = "rgba(168, 85, 247, 0.2)";
              e.currentTarget.style.borderColor = "var(--accent-purple, #a855f7)";
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.color = "var(--text-secondary)";
              e.currentTarget.style.background = "rgba(255, 255, 255, 0.05)";
            ...
            aria-label={`${member.name} GitHub 2`}
          >
            <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
            </svg>
...
function HiringCard({ name, role, bio, skills, cta }: { name: string; role: string; bio: string; skills: string[]; cta: string }) {
  return (
    <div
